...
def convert_action(action_dict):
	...
	if len(action_dict.keys()) > 1:
		actions_ng = [x for x in action_dict.values() if x is not None]
		if any(('open' in x) or ('grab' in x) for x in actions_ng):
			# continue
			object_to_agents = {}
			for agent_id, action in action_dict.items():
				if action is not None and ('open' in action or 'grab' in action):
					try:
						obj_id = action.split('(')[1].split(')')[0]
						if obj_id not in object_to_agents:
							object_to_agents[obj_id] = []
						object_to_agents[obj_id].append(agent_id)
					except:
						# if the action is not related to an object, continue
						continue
			final_agents = []
			for agent_id, action in action_dict.items():
				if action is not None and 'open' not in action and 'grab' not in action:
					final_agents.append(agent_id)
			for obj_id, agents in object_to_agents.items():
				if len(agents) > 1:
					selected_agent = random.choice(agents)
					final_agents.append(selected_agent)
					collision = True
				else:
					final_agents.append(agents[0])
					
			agent_do = final_agents
