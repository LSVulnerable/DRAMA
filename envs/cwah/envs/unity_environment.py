...
class UnityEnvironment(BaseUnityEnvironment):
	...
	def reset(self, environment_graph=None, task_id=None, reset_seed=None):
		...
		# self.task_goal = {i: task['task_goal'][i] for i in range(len(task['task_goal']))}
		# print('task_goal: ', self.task_goal)
		logger.info(f'task_goal: {env_task["task_goal"][0]}')
		if 'total_goal' in env_task.keys():
			logger.info('total_goal: ', env_task['total_goal'])
		...
		num_rooms = len(room_info)
		room_indices = list(range(num_rooms))
		self.rnd = random.Random(self.seed + int(time.time()) + task_id)
		if self.num_agents <= num_rooms:
			sampled_indices = self.rnd.sample(room_indices, self.num_agents)
		else:
			unique_indices = self.rnd.sample(room_indices, num_rooms) 
			extra_indices = [self.rnd.choice(room_indices) for _ in range(self.num_agents - num_rooms)]
			sampled_indices = unique_indices + extra_indices
	...
	def step(self, action_dict):
		if self.steps > 245:
			logger.info("Warning: too many steps")
		K = 500
		actions = {key: utils.get_action_name(value) for key, value in action_dict.items()}
		...
		for i, action in actions.items():
			...
			if action == 'send_message':
				...
				if len(saying[i]) > K:
					saying[i] = saying[i][:K]
					logger.info("Message too long, truncating to {}".format(K))
			...
			if action == 'walktowards' and self.keep_move_steps[i] > 6:
				if self.range_distance(self.location[i][max(0, self.steps - 4) : self.steps + 1]) < 0.5:
					...
					logger.info("Randomly changing walktowards action since the agent might get stuck")
					random_flag = True
			self.keep_move_steps[i] = self.keep_move_steps[i] + 1 if action == 'walktowards' and random_flag == False else 0

		# logger.info(f"Action dict verbose: {action_dict_verbose}")

		script_list, _ = utils.convert_action(action_dict_tobe_changed)
		script_list_verbose, collision = utils.convert_action(action_dict_verbose)
		if collision:
			self.collision_num += 1
			logger.info(f"Collision detected! Total collision num: {self.collision_num}")

		if isinstance(script_list, str):
			script_list = [script_list]
		elif isinstance(script_list, tuple):
			script_list = list(script_list)
		elif not isinstance(script_list, list):
			script_list = list(script_list)
		
		# logger.info(f"Converted script list: {script_list_verbose} ")
