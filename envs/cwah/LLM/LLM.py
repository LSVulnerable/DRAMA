...
class LLM:
	...
	def parse_answer(self, available_actions, text):
		...
		for i in range(len(available_actions)):
			...
			if f"{option} " in text or act in text or name in text or id in text:
				return action
		logger.info("WARNING! No available action parsed!!! Random choose one")
	...
	def get_available_plans(self, grabbed_objects, unchecked_containers, ungrabbed_objects, message, room_explored):
		...
		available_plans = []
		for room in self.rooms:
			if (room_explored is None or room_explored[room]) and unchecked_containers[room] is not None and ungrabbed_objects[room] is not None:
				continue
	...
	def run(self, current_room, grabbed_objects, satisfied, unchecked_containers, ungrabbed_objects, goal_location_room, action_history, dialogue_history, teammate_grabbed_objects, teammate_last_room, room_explored = None, steps = None, subgoal = None, state_prompt = None):
		info = {}
		...
		prompt = prompt.replace('$SUBGOAL$', str(subgoal))
		...
		if num == 0 or (message is not None and num == 1):
			logger.info("Warning! No available plans!")
			plan = None
		...
		for _ in range(3):
			try:
				outputs, usage = self.generator([{"role": "user", "content": prompt + state_prompt}] if self.chat else prompt, self.sampling_params)
				# logger.info(f"raw outputs: {outputs}")
				response_clean = outputs.strip()
				if response_clean.startswith('```json'):
					response_clean = response_clean[7:-3].strip()
				if response_clean.endswith('```'):
					response_clean = response_clean[3:-3].strip()
				logger.info(f"outputs—json: {response_clean}")
				outputs_json = json.loads(response_clean)
		...
		info['input_usage'] = usage.get('prompt_tokens', 0)
		info['output_usage'] = usage.get('completion_tokens', 0)
