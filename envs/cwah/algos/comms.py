...
def select_target(prompt, autogen_agent, history, cur_history, info: Dict, llm_agent: LLM_agent):
	...
	for _ in range(5):
		...
		try:
			...
			json_start = "{"
			json_end = "}"
			if outputs.startswith(prefix) and outputs.endswith(suffix):
				start_index = len(prefix)
				if outputs[start_index:].startswith("\n"):
					start_index += 1
				outputs = outputs[start_index:-len(suffix)].strip()
			elif outputs.startswith(prefix) and outputs.endswith(json_end):
				start_index = len(prefix)
				if outputs[start_index:].startswith("\n"):
					start_index += 1
				outputs = outputs[start_index:-len(json_end)].strip()
			elif outputs.startswith(json_start) and outputs.endswith(suffix):
				end_index = outputs.rfind(json_end)
				if end_index == -1:
					raise ValueError("Invalid JSON format: missing closing brace.")
				outputs = outputs[len(json_start):end_index].strip()
...
def format_dialog(name, history: Dict, cur_history):
	...
	for idx, messages in enumerate(his):
		...
		for message in messages:
			if '[SYSTEM]' in message and ('has disconnected' in message):
				prompt += f"[IMPORTANT]{message}\n"
			else:
				parts = message.split(":")
				names = parts[0].strip()
...
def communicate(names, selector_prompts, history, info: Dict, llm_agents: List[LLM_agent], converse_agents: List[AssistantAgentCoT], random_start_comm=False, log_thoughts=True, visualize=False, fig_folder_name="fig/comms"):
	...
	llm_config_list = []
	speak_order = list(range(len(converse_agents)))
	communicate_edges = []
