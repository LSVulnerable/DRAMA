...
class LLM_agent:
	...
	def __init__(self, agent_id, args):
		...
		self.status = "active"  # can be 'active', 'disconnected'

		self.guardian_role_table = []
		self.input_usage = 0
		self.output_usage = 0
	...
	def LLM_plan(self, subgoal=None, state_prompt=None):
		if len(self.grabbed_objects) == 2:
			return f"[goput] {self.goal_location}", {}

		return self.LLM.run(self.current_room, [self.id2node[x] for x in self.grabbed_objects], self.satisfied, self.unchecked_containers, self.ungrabbed_objects, self.id_inside_room[self.goal_location_id], self.action_history, self.dialogue_history, self.teammate_grabbed_objects, [self.id_inside_room[teammate_agent_id_item] for teammate_agent_id_item in self.teammate_agent_id], None, self.steps, subgoal, state_prompt)
	...
	def obs_processing(self, observation, goal):
		...
		# logger.info(f"satisfied: {satisfied}, unsatisfied: {unsatisfied}")
		if len(satisfied) > 0:
		...
		self.reachable_objects = []
		self.id2node = {x['id']: x for x in observation['nodes']}
	...
	def get_action(self, observation, goal, dialogue_history=None, subgoal = None, reexecute = False):
		"""
		...
		while action is None:
			if self.plan is None:
				...
				if LM_times > 5:
					...
					plan, a_info = self.LLM_plan(subgoal, state_prompt)
					# if self.debug:
					# 	print(LM_times, "plan: ", plan)
					if plan is None: # NO AVAILABLE PLANS! Explore from scratch!
						logger.info("No more things to do!")
						plan = f"[wait]"
				self.plan = plan
				a_info.update({"steps": self.steps})
				info.update({"LLM": a_info})
				self.input_usage += a_info.get('input_usage', 0)
				self.output_usage += a_info.get('output_usage', 0)
				info['input_usage'] = self.input_usage
				info['output_usage'] = self.output_usage
				LM_times += 1
	...
	def reset(self, obs, containers_name, goal_objects_name, rooms_name, room_info, goal):
		...
		self.id_inside_room = {self.goal_location_id: self.rooms_name[:]}
		self.teammate_names = self.agent_names[:self.agent_id - 1] + self.agent_names[self.agent_id:]
		self.teammate_agent_id = [i+1 for i in range(self.args.agent_num)]
		self.teammate_agent_id.remove(self.agent_id)
	...
	def generate_state_prompt(self):
		ego_id = self.agent_id
		layout_prompt = f"Unchecked containers in my({ego_id}) room({self.current_room['class_name']}):{self.unchecked_containers}\n"
		hold_prompt = f"State of my({ego_id}) grabbed objects: {[self.id2node[x] for x in self.grabbed_objects]}.\n"
		reachable_prompt = f"Reachable objects in my({ego_id}) room({self.current_room['class_name']}): {self.reachable_objects}.\n"
		teammate_hold_prompt = f"The teammates in the same room with me({ego_id}) hold objects: {self.teammate_grabbed_objects}.\n"
		goal_prompt = f"satisfied objects: {self.satisfied}.\n, unsatisfied objects: {self.unsatisfied}.\n"
		state_prompt = layout_prompt + hold_prompt + reachable_prompt + teammate_hold_prompt + goal_prompt
		return state_prompt
